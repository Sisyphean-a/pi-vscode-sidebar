...
**Tech Stack:** TypeScript, VSCode Extension API, Vitest, @vscode/test-electron, rolldown, pnpm, GitHub Actions
...
3. 当前自动化通过：`pnpm lint`、`pnpm typecheck`、`pnpm exec vitest run`、`pnpm build`。
...
Run: `pnpm exec tsx test/e2e/run-e2e.ts`  
...
Run: `pnpm exec vitest run test/unit/view/extension-ui.test.ts`  
Expected: FAIL
...
Run: `pnpm exec vitest run test/integration/sidebar-controller.test.ts`  
Expected: PASS
...
Run: `pnpm exec vitest run test/unit/host/logger.test.ts test/unit/host/controller.test.ts`  
Expected: PASS
...
Run: `pnpm exec vitest run test/unit/bridge/server.test.ts test/unit/bridge/utils.test.ts`  
Expected: PASS
...
Run: `pnpm run test:perf`（首次可允许记录模式，不设硬阈值阻断）  
